...
class Style
{
    ...
    function length_in_pt($length, $ref_size = null)
    {
        ...
        foreach ($length as $l) {
            ...
            if (is_string($l) && ($i = mb_strpos($l, "px")) !== false) {
            ...
            if (is_string($l) && ($i = mb_strpos($l, "em")) !== false) {
                $ret += (float)mb_substr($l, 0, $i) * $this->__get("font_size");
                continue;
            }

            if (is_string($l) && ($i = mb_strpos($l, "cm")) !== false) {
                $ret += (float)mb_substr($l, 0, $i) * 72 / 2.54;
                continue;
            }

            if (is_string($l) && ($i = mb_strpos($l, "mm")) !== false) {
                $ret += (float)mb_substr($l, 0, $i) * 72 / 25.4;
                continue;
            }

            // FIXME: em:ex ratio?
            if (is_string($l) && ($i = mb_strpos($l, "ex")) !== false) {
                $ret += (float)mb_substr($l, 0, $i) * $this->__get("font_size") / 2;
                continue;
            }

            if (is_string($l) && ($i = mb_strpos($l, "in")) !== false) {
                $ret += (float)mb_substr($l, 0, $i) * 72;
                continue;
            }

            if (is_string($l) && ($i = mb_strpos($l, "pc")) !== false) {
