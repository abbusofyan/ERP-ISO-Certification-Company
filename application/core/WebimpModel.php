...
class WebimpModel extends CI_Model
{
    ...
    protected $_temporary_with_deleted_relate = false;
    ...
    public function get_by()
    {
        ...
        $row = $this->trigger('after_relate', $row);
    ...
    public function get_all()
    {
        ...
        foreach ($result as $key => &$row) {
            ...
            $row = $this->trigger('after_relate', $row, ($key == count($result) - 1));
        }
    ...
    /**
     * Explicitly define which columns to retrieve.
     */
    public function select($columns = '*', $escape = true)
    {
        $this->_database->select($columns, $escape);
        return $this;
    }
    ...
    public function delete($id)
    {
        ...
        $this->trigger('after_delete', array($id, $result));
    ...
    public function delete_by()
    {
        ...
        $where = $this->trigger('before_delete_by', $where);
        ...
        $this->trigger('after_delete_by', $result);
    ...
    /**
     * Set the primary key for model.
     *
     * @access public
     * @param str $str
     * @return obj
     */
    public function set_primary_key($str)
    {
    ...
    public function relate($row)
    {
        ...
        foreach ($this->belongs_to as $key => $value) {
            ...
            if (in_array($relationship, $this->_with)) {
                ...
                if ($this->_temporary_with_deleted_relate) {
                    $this->{$options['model']}->with_deleted_relate();
                }
    ...
    /**
     * Filter options via where.
     *
     * @access public
     * @return void
     */
    public function dropdown_by()
    {
        $args = func_get_args();
        ...
        switch (count($args)) {
            case 3:
                if ($args[2]) {
                    $this->_database->where($args[2]);
                }

                $options = $this->dropdown($args[0], $args[1]);
                break;
            case 2:
                if ($args[1]) {
                    $this->_database->where($args[1]);
                }

                $options = $this->dropdown($args[0]);
                break;
            case 1:
                $options = $this->dropdown($args[0]);
                break;
    ...
    /**
     * Don't care about soft deleted rows on the relate
     */
    public function with_deleted_relate()
    {
        $this->_temporary_with_deleted_relate = true;
        return $this;
    }
