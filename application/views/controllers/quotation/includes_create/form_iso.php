...
$(document).on('change', '#select-address', function(e) {
	...
	props.forEach((item, i) => {
		...
		`;
		$('.selected-address-section:last').append(template)
		$(".address-country").select2({
			data: data,
		})
		$(".address-country").select2("val", value.country);
	});
	countTotalSite()
})
...
$(document).on('change', '.certification-scheme-iso', function() {
	...
	if (this.value) {
		...
		$('.certification-scheme-iso').each(function() {
			if (this.value && selected_certification_scheme.includes(parseInt(this.value))) {
				$(this).select2('val', '')
				return Swal.fire({
					icon: 'error',
					title: 'Error!',
...
$(document).on('change', '#received_prev_reports', function() {
	var received_prev_reports = $(this).val();
	if (received_prev_reports == 'No') {
		Swal.fire({
			icon: 'warning',
			text: 'You are not able to proceed as a transfer quotation. Please change the certification cycle.',
			customClass: {
				confirmButton: 'btn btn-success'
			}
		})
		$('#iso_certificate_cycle').val('').trigger('change');
