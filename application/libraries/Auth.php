    ...
    */
    ...
    protected function init()
    {
        if ($this->CI->session->has_userdata("user_id")) {
            $this->userID = $this->CI->session->user_id;
            $this->userName = $this->CI->session->name;
            $this->roles = $this->CI->session->group_id;
    ...
    public function userPermissions()
    {
        ...
        ->select("permissions.*")
        ->from("permissions")
        ->join("group_permissions", "permissions.id = group_permissions.permission_id", "inner")
        ->where_in("group_permissions.group_id", $this->roles())
        ->where(array("permissions.status" => 1, "deleted_at" => null))
        ->group_by("group_permissions.permission_id")
        ->get()->result_array());
    }
    ...
    public function route_access()
    {
		...
		if ($this->CI->group_model->hasFullAccess($user['group_id'])) {
			return true;
		}
		...
		if ($method == '' || $method == 'view') {
			$method = 'read';
		} elseif ($method == 'update' || $method == 'edit') {
			$method = 'update';
		} elseif ($method == 'create' || $method == 'store') {
			$method = 'create';
		}

        $routeName = $method . "-" . $controller;
		...
		$this->CI->set_alert("Action not allowed!", 'error', true);
