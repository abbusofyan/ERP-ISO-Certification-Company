				...
				<div class="row">
					<div class="col-6">
						<div class="form-group">
							...
							<textarea class="form-control" name="iso-scope" title="Certification Scope" required rows="5"><?= ($quotation->scope) ?></textarea>
						</div>
					</div>
					<div class="col-2">
						<div class="form-group">
							<label for="referred_by">Number of Sites <span class="text-danger">*</span></label>
							<input type="number" class="form-control" id="number-of-sites" value="<?= $quotation->num_of_sites ?>" name="iso-num_of_sites" readonly>
						</div>
					</div>
				</div>
							...
							<div class="form-group">
								...
								<select class="form-control select2 select-select2 certification-scheme-iso" title="Certification Scheme" name="iso-certification_scheme[]" required>
									...
									<?php foreach ($certification_schemes as $scheme): ?>
										<option value="<?= $scheme->id ?>" <?= $scheme->name == $selected_scheme ? 'selected' : '' ?>><?= $scheme->name ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
							...
							<div class="form-group">
								...
								<select class="form-control select2 select-select2 accreditation-iso" title="Accreditation" name="iso-accreditation[]" required>
									...
									<?php foreach ($accreditations as $acc): ?>
										<option value="<?= $acc->id ?>" <?= $acc->name == $quotation->accreditation_arr[$key] ? 'selected' : '' ?>><?= $acc->name ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
